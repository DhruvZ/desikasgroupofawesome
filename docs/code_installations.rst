...
hyperion (Desika's fork, commit 1d0d133)

NOTE - if you get an error trying to run the configure step on EL9 that looks like ``configure: error: Could not determine MPI compiler: ifx``, you likely need to change line 2012 of configure in hyperion from ``elif test "$mpi_compiler" == ifort`` to ``elif test "$mpi_compiler" == ifx`` for the configure to be successful::

  >cd $HOME
  >git clone https://github.com/dnarayanan/hyperion.git
  >cd hyperion
  >git checkout powderday
  >git submodule init
  >git submodule update
